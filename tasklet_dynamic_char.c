...
static int __init my_driver_init(void)
{
   ...
   mutex_init(&my_mutex);	
	...
	char_thread1 = kthread_create(my_thread_fun1,NULL,"my char thread1");
	if(char_thread1)
	{
	...
	{
		printk(KERN_INFO "Thread1 creation failed\n");
		goto r_device;
	} 
	...
	{
		printk(KERN_INFO "Thread2 creation failed\n");
		goto r_device;
	} 
	...
	printk(KERN_INFO "Character device init is successfull\n");
	return 0;
...
static void __exit my_driver_exit(void)
{
	...
	cdev_del(&my_cdev);
	unregister_chrdev_region(dev,1);
	printk(KERN_INFO "kernel driver removed\n");
}
